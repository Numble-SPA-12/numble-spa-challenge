...
export default class RandomImageSelector extends Component {
  template() {
    ...
    const containerStyle = `w-main flex align-cetner justify-center border rounded-lg h-80 hover:bg-slate-100 ease-in duration-150 cursor-pointer`;
    const imageStyle = `select-none rounded-lg w-full hover:opacity-80 ease-in duration-150 ${
      image ? 'object-cover ' : 'object-none'
    }`;

    return `
      <div class='write-image-container ${containerStyle}'>
        <img src="${
